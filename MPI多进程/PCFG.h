...
public:
    ...
    // MPI serialization: pack model into a byte buffer for broadcast
...
// PT probability comparator: highest prob first (for std::priority_queue max-heap)
struct PTProbLess {
    bool operator()(const PT &a, const PT &b) const { return a.prob < b.prob; }
};

// PT serialization for MPI transfer (advanced)
...
{
public:
    // MultiQueue implements a relaxed concurrent priority queue.
    MultiQueue priority;

    // Simple single-threaded priority queue (for MPI basic: rank 0 only)
    std::priority_queue<PT, std::vector<PT>, PTProbLess> simple_heap;
    ...
    // 优先队列的初始化 (uses MultiQueue)
    void init();

    // Simple queue init: seeds std::priority_queue with all ordered_pts
    void init_simple();

    // Simple queue operations
    bool PopMaxSimple(PT &out);
    void PushSimple(const PT &pt);
    bool EmptySimple() const;

    // MPI-aware init: each rank takes a round-robin subset of ordered_pts
    void init_mpi(int rank, int nprocs);

    // 对优先队列的一个PT，生成所有guesses
    void Generate(PT pt);

    // Generate a chunk of guesses: values[start..end) with prefix
    // Used by MPI basic: all ranks process their share of a single segment
    void GenerateChunk(const std::string& prefix, const segment* seg,
                       int start, int end,
                       std::vector<std::string>& out, int& out_count);

    // Generate ALL guesses for a PT into a local buffer (full expansion)
    // Used by MPI advanced: each rank fully processes its assigned PT
    void GenerateLocal(PT pt, std::vector<std::string>& out, int& out_count);
